...
{
  //radius of ball;
  float radius = CircleTri_radius(pendingTriangles.element());
  println(radius);
  if(choice == 5)
  radius += 2000;
  else
  ...
  pt absolute_center = CircleTri(pendingTriangles.element());
  absolute_center.z = (h_ceiling + h_floor) / 2.0f;
  
  while(pendingTriangles.size() > 0)
  {
    ...
    NextTriangle(temp_tri,AB,temp_tri.c,vertex,radius,absolute_center);
    
   if(!inEdgeBuffer(visited_edge,BC))
    NextTriangle(temp_tri,BC,temp_tri.a,vertex,radius,absolute_center);
    
    if(!inEdgeBuffer(visited_edge,AC))
    NextTriangle(temp_tri,AC,temp_tri.b,vertex,radius,absolute_center);
  }
